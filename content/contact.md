---
title: "Contact"
subtitle: "Let's discuss how Photometrics AI can help your municipality"
description: "Schedule a demo of Photometrics AI street lighting optimization. Learn how municipalities and utilities achieve 40% energy savings through existing networked lighting controls—no hardware required."
layout: "contact"
---

Ready to see how Photometrics AI can transform your street lighting?

## Get in Touch

**Ari Isaak, Founder**

- **Email:** [ari@photometrics.ai](mailto:ari@photometrics.ai)
- **Phone:** [858-633-6447](tel:858-633-6447)

---

## What to Expect

1. **Discovery call** — We'll learn about your lighting infrastructure, NLC system, and goals
2. **Data review** — We'll assess your asset data and integration requirements
3. **Pilot proposal** — Customized scope for your municipality or service territory
4. **Deployment** — Typically weeks, not months

---

## Frequently Asked Questions

**Do I need a certain NLC system?**
We're currently integrated with 2 major lighting control platforms. Contact us to confirm compatibility.

**What data do you need to get started?**
Asset inventory (locations, fixture types), IES photometric files, and API access to your NLC.

**What's the typical pilot size?**
Most pilots start with 2,000-5,000 lights to demonstrate value before scaling.

**Is there a hardware requirement?**
No. Photometrics AI is software-only. If you have networked lighting controls, you likely have everything you need.
